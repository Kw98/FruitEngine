...
namespace g_reg {
	...
	class GRegister {
		public:
			...
			template<typename ...T>
			std::vector<g_reg::entity_s<T ...>>	global_view(const std::vector<std::size_t>& entt) {
				std::vector<g_reg::entity_s<T...>>	data;
				data.reserve(entt.size());
				for (auto&& id : entt) {
					if (cpool_has<T...>(id))
						data.push_back({ id, safe_view<T...>(id) });
				}
			...
			inline bsparse_set* get_native_pool_by_type_index(const std::type_index& ti) { return _pool[ti].get(); }
